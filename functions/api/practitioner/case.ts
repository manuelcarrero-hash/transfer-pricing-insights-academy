...
async function assessmentEvidence(db:Env['CERTIFICATES_DB'],eligibilityId:string){
...
export async function onRequestGet(context:{env:Env;request:Request}){
  ...
  const assessment=await assessmentEvidence(db,eligibilityId);
  if(!assessment||assessment.passed!==1)return json({error:'assessment_not_passed'},403);
  return json({eligibilityId,facts:practitionerCaseFacts,passScore:CONSULTANT_CASE_PASS_SCORE,questions:practitionerCaseQuestions.map(({id,domain,prompt,options,correct:_,explanation:__,...rest})=>({id,domain,prompt,options,...rest}))});
}
