...
export function CourseProgressCard({ level, code, course, lessons, progress }: CourseProgressCardProps) {
  ...
  const courseComplete = completed === lessons.length;
  const primaryHref = courseComplete ? href : `${href}/lesson/${resume}`;
  const primaryLabel = courseComplete ? 'Ver curso' : progress.lastLesson ? 'Continuar donde me quedé' : 'Comenzar curso';
  ...
  return (
    <section className="progress-card" aria-labelledby={titleId}>
      ...
      <div className="progress-actions">
        <Link className="button primary" to={primaryHref}>{primaryLabel}</Link>
        {!courseComplete && <Link className="button secondary" to={href}>Ver curso</Link>}
      </div>
      {progress.lastLesson && !courseComplete && <p className="resume-note">Última lección visitada: <strong>{progress.lastLesson}. {lessons[resume - 1]?.title}</strong></p>}
    </section>
  );
}
