...
function storedCertificateId() {
  try {
    const raw = localStorage.getItem(CERTIFICATE_KEY);
    const parsed = raw ? JSON.parse(raw) as { certificateId?: string } : null;
...
export function JuniorCertificatePage() {
  ...
  const certificateId = searchParams.get('id')?.trim() || storedCertificateId();
