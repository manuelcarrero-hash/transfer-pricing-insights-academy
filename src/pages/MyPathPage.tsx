...
export function MyPathPage({showLocalNote=true,semiSeniorExtension,seniorExtension}:MyPathPageProps){
 ...
 return <section className="section my-path-page"><div className="container path-container">
  ...
  <div className="path-journey" aria-label="Niveles de la Academy">
   <PathLevelSection number="01" title="Junior" subtitle="Fundamentos" status={consultantUnlocked?'complete':'current'} summary={consultantUnlocked?'5 cursos completados · cierre de nivel aprobado':`${juniorCourses.filter(([, ,lessons,p])=>p.completedLessons.length===lessons.length).length} de 5 cursos completados`} defaultOpen={!consultantUnlocked}>
   ...
   <PathLevelSection number="04" title="Senior Knowledge" subtitle="Criterio y juicio profesional" status={!seniorUnlocked?'locked':'current'} summary={!seniorUnlocked?'Completa Semi Senior para desbloquear':'Nivel avanzado disponible'} defaultOpen={seniorUnlocked}>
    {seniorExtension}
   </PathLevelSection>
  </div>
