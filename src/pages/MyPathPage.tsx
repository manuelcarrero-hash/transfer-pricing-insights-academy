...
export function MyPathPage() {
  ...
  useEffect(() => { const sync = () => { setJ2Progress(getCourseProgress('J2', j2Lessons.length)); setJ3Progress(getCourseProgress('J3', j3Lessons.length)); setJ4Progress(getCourseProgress('J4', j4Lessons.length)); setJ5Progress(getCourseProgress('J5', j5Lessons.length)); }; window.addEventListener(courseProgressEventName, sync); window.addEventListener('storage', sync); return () => { window.removeEventListener(courseProgressEventName, sync); window.removeEventListener('storage', sync); }; }, []);
  const courses = [
