...
test.describe('RC1-R resource integration', () => {
  ...
  test('client onboarding case lab is an interactive experience with local progress', async ({ page }) => {
    await page.goto('/labs/client-onboarding');
    await expect(page.getByRole('heading', { name: 'Client Onboarding Case Lab' })).toBeVisible();
    await expect(page.getByText('0 de 24 verificaciones completadas')).toBeVisible();

    const firstCheck = page.getByRole('checkbox').first();
    await firstCheck.check();
    await expect(page.getByText('1 de 24 verificaciones completadas')).toBeVisible();
